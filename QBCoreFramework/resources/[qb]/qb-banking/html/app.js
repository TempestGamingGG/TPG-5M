...
function ResetModals(type, sSteamID) {
	...
	if (sSteamID && sSteamID !== "undefined") GlobalSteamID = sSteamID;
}
...
function AddAccount(account_name, account_type, bal, ply_Name, steamID) {
	if (playerName == null || playerName === "") playerName = ply_Name;
	...
	if (ValidAccounts[temp_Name]) return UpdateAccount(temp_Name, bal);

	$("#Player_Accounts").append(
		"<div id='" +
			temp_Name +
			"'\
        <div class='card bg-secondary'>\
			...
			account_name +
			((relang[account_type] === "personal" &&
				" <i OnClick='EditAccount(\"" +
					transferAcc +
					"\")' class='justify-content-end right fa fa-cog account_settings'></i>") ||
				"") +
			"</h5>\
                <p class='card-subtitle note'>" +
			account_type +
			"</p>\
\
                <div class='row'>\
...
\
                ...
                <div class='d-flex justify-content-between'>\
                    <button type='button' class='btn btn-light mb-2 text-muted deposit' data-toggle='modal' data-target='#DepositModal' onClick='ResetModals(\"" +
			transferAcc +
			'", "' +
			steamID +
			"\")'>DEPOSIT</button>\
                    <button type='button' class='btn btn-light mb-2 text-muted withdraw' data-toggle='modal' data-target='#WithdrawModal' onClick='ResetModals(\"" +
			transferAcc +
			'", "' +
			steamID +
			"\")'>WITHDRAW</button>\
                    <button type='button' class='btn btn-light mb-2 text-muted transfer' data-toggle='modal' data-target='#TransferModal' onClick='ResetModals(\"" +
			transferAcc +
			'", "' +
			steamID +
			"\")'>TRANSFER</button>\
...
function AddTransaction(
	...
	let ColStr =
		(amount.toString().charAt(0) === "-" && "expense") || "addition";

	let str =
		(amount.toString().charAt(0) === "-" &&
			"-$" +
				parseInt(
					amount.toString().substring(1, amount.length),
				).toLocaleString()) ||
        ...
        <div class='card bg-secondary mb-2'>\
			...
			"</p>\
                    ...
                    <div class='col d-flex justify-content-end'>\
                        <p class='header-title'><span class='badge bg-primary'>Bank Transfer</span> <> <span class='badge bgdark1'>" +
			lang[why] +
			"</span></p>\
                    </div>\
                    <hr/>\
...
function OpenATM(data, friends, transactions, name) {
	if (data && data !== null) {
		...
		for (var i = 0; i < tbl.length; i++) {
			let tTbl = tbl[i];
			AddAccount(
				(tTbl.type === "business" && tTbl.name) || "Personal Account",
				(lang[tTbl.type] && lang[tTbl.type]) || tTbl.type,
				tTbl.amount,
				name,
			);
		}
...
function EditAccount(clss) {
	if (clss != "personal")
		return TropixNotification(
			"Company accounts have no settings yet, sorry!",
			"error",
		);
...
Listeners["notification"] = function (data) {
	$("#notifaction_type").text(
		(data.msg_type && data.msg_type == "error" && "Error!") || "Success!",
	);
	...
	if (data) {
		...
		for (var i = 0; i < auths.length; i++) {
            ...
            <td>" +
					TimeSince +
					"(s) ago</td>\
            <td><button class='btn btn-danger btn-sm' onclick='confirmRemove(\"" +
					v.target_identifier +
					'", "' +
					v.target_name +
					"\")'>Remove</button></td>\
...
function closeNotification(br) {
	if (br) return $("#editAccountModal").modal("toggle");
...
$(function () {
	window.addEventListener("message", (event) => {
		...
		if (Listeners[type]) Listeners[type](event.data);
	});
...
function CloseUIPls() {
	...
	$("#bankui").fadeTo(10, 0, () =>
		$.post("http://" + folder_name + "/CloseATM", JSON.stringify({})),
	);
}
