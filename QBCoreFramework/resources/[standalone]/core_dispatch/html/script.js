...
function sendCall(id, call, cool) {
	if (call.type == "call") {
		var base =
			'    <div class="rgba-background clearfix colelem alert  call scale-up-right" id="call_' +
			id +
			'" data-x="' +
			call.coords[0] +
			'" data-y="' +
		...
		for (i = 0; i < Object.keys(call.extraInfo).length; i++) {
			base =
				...
				'      <div class="grpelem" id="u1235"><i class="fas ' +
				call.extraInfo[i].icon +
				'"></i><!-- simple frame --></div>' +
				'      <div class="clearfix grpelem" id="u1236-4"><!-- content -->' +
				"       <p>" +
				String(call.extraInfo[i].info).toUpperCase() +
				"</p>" +
				"      </div>" +
				"      </div>";
		}
		...
		for (i = 0; i < Object.keys(call.respondingUnits).length; i++) {
			base =
				base +
				'<div class="unit ' +
				call.respondingUnits[i].type +
				'">' +
				call.respondingUnits[i].unit +
				"</div>";
		}
		...
		var base =
			'<div class="rgba-background clearfix colelem alert scale-up-right message" id="call_' +
			id +
			'" data-phone="' +
			call.phone +
			'" data-caller="' +
		...
		for (i = 0; i < Object.keys(call.respondingUnits).length; i++) {
			base =
				base +
				'<div class="unit ' +
				jobInfo[call.respondingUnits[i].type].color +
				'" id="unit_' +
				call.respondingUnits[i].unit +
				'">' +
				call.respondingUnits[i].unit +
			...
			if (value.job[i] == job) {
				if (value.type == "call") {
					if (!$("#call_" + key).length) {
						...
						for (
							i = 0;
							i < Object.keys(value.extraInfo).length;
							i++
						) {
							base =
								base +
								'     <div class="clearfix colelem" id="pu1235"><!-- group -->' +
								'      <div class="grpelem" id="u1235"><i class="fas ' +
								value.extraInfo[i].icon +
								'"></i><!-- simple frame --></div>' +
								'      <div class="clearfix grpelem" id="u1236-4"><!-- content -->' +
								"       <p>" +
								String(value.extraInfo[i].info).toUpperCase() +
								"</p>" +
								"      </div>" +
								"      </div>";
						}
						...
						for (
							i = 0;
							i < Object.keys(value.respondingUnits).length;
							i++
						) {
							base =
								base +
								'<div class="unit ' +
								jobInfo[value.respondingUnits[i].type].color +
								'" id="unit_' +
								value.respondingUnits[i].unit +
								'">' +
								value.respondingUnits[i].unit +
								"</div>";
						}
...
function populateUnits(units) {
	...
	for (const [key, value] of Object.entries(units)) {
		...
		base =
			base +
			'<div class="rounded-corners unitgroup" id="u1408">' +
			'<i class="fas ' +
			value.type +
			' vehiclesign"></i>' +
			'<div class="unitgrid">';
...
function OpenBase() {
	var base =
		'   <div class="clearfix grpelem " id="pu1129"><!-- group -->' +
...
function forwardCall(num) {
	for (const [key, value] of Object.entries(jobInfo)) {
		if (value.column == num) {
			$.post(
				"https://core_dispach/forwardCall",
				JSON.stringify({
...
function changeToBusy() {
	$.post(
		"https://core_dispach/updatestatus",
		JSON.stringify({
			id: id,
			status: "Busy",
		}),
...
function dismissCall() {
	...
	acceptedCall = "";
	$.post(
		"https://core_dispach/dismissCall",
		JSON.stringify({
			id: currentElement.id,
		}),
	);
...
function requestBackup() {
	...
	for (const [key, value] of Object.entries(jobInfo)) {
		if (value.column == col) {
			$.post(
				"https://core_dispach/reqbackup",
				JSON.stringify({
					job: key,
					requester: jobInfo[job].name,
					firstname: firstname,
					lastname: lastname,
				}),
			);
...
window.oncontextmenu = function (event) {
	...
	if (element != null) {
		...
		if (columnMenu) {
			...
			if (
				jobInfo[job].canRequestLocalBackup &&
				col == jobInfo[job].column
			) {
				html =
					html +
					'    <a onclick="requestBackup()">Request Backup</a>';
			}
			if (
				jobInfo[job].canRequestOtherJobBackup &&
				col != jobInfo[job].column
			) {
				html =
					html +
			...
			html =
				html + '    <a onclick="changeToPatrol()">Change to patrol</a>';
		} else {
			if (acceptedCall == "") {
				html = html + '    <a onclick="acceptCall()">Accept Call</a>';
				if (jobInfo[job].canRemoveCall) {
					html =
						html + '    <a onclick="removeCall()">Remove Call</a>';
				}
				if ($(element).hasClass("message")) {
					html =
						html + '    <a onclick="copyNumber()">Copy number</a>';
				}
			}
